...
6. **Classify story type** — for each issue, determine `story_type`:
   - `technical` if type is `type/tech-debt`, `type/docs`, `type/infra`, or `type/refactoring`
   - `functional` if type is `type/feature` or `type/bug`
   - Default to `functional` if uncertain
   - Record `story_type` in the triage table for every issue

**Output**: structured triage table per issue (includes `story_type` column)
...
|---|---|---|---|---|---|---|
| 42 | Add eligibility check for young drivers | feature | P1 | M | functional | Core flow |
| 43 | Fix validation error on driver age field | bug | P0 | S | functional | Blocking submission |
| 44 | Refactor premium calculation engine | tech-debt | P2 | L | technical | No Gherkin in DISTILL |
