...
### 0. STORY TYPE CHECK (Mandatory First Step)

Read `story_type` from the triage report at `.skraft/sdlc/discover/triage-*.md` (most recent) or from the issue labels (`type/tech-debt`, `type/infra`, `type/refactoring`, `type/docs`).

- If `story_type == technical`: **skip sections 3 (GHERKIN WRITING) and 4 (TEST PLAN)** — jump directly to section 5 (IMPLEMENTATION PLAN). Technical stories do not produce `.feature` files.
- If `story_type == functional`: execute all sections 1–6 in full.
...
Write artefacts to `.skraft/sdlc/distill/`:

**Functional story** (`story_type == functional`):
...
**Technical story** (`story_type == technical`):
- `impl-plan-{story}.md` — sequenced implementation plan only (no `.feature`, no test-plan)
